...
module Make (Ch: MParser_Sig.Channel) (Rx: MParser_Sig.Regexp) = struct
  ...
  let io_input chn buffer pos length =
    if 0 <= pos && pos + length <= String.length buffer then
      ...
      let stop = ref false in
        while not !stop do
          let chars =
            Ch.read chn buffer (pos + !chars_read) (length - !chars_read)
          in
          if chars > 0 then
            chars_read := !chars_read + chars
          else
            stop := true
        done;
        !chars_read
  ...
  *)
  ...
  let read_string s pos maxlen =
    if not (is_valid_pos s pos) then "" else
  ...
  let match_string s pos str =
    if not (is_valid_pos s pos) then str = "" else
      let len = String.length str in
